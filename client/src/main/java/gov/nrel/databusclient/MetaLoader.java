...
public class MetaLoader {
	...
	private void loadDevice(DefaultHttpClient httpclient, Map<String, String> row, String streamTable) {
		...
		String deviceDescription = row.get("description");
		String owner = row.get("owner");
		String endUse = row.get("endUse");
		...
		d.setDeviceDescription(deviceDescription);
		d.setOwner(owner);
		d.setEndUse(endUse);
		addDevice(d);
		...
		for(Map<String, String> srow : streams) {
			...
			String virtual = srow.get("virtual");
			String processed = srow.get("processed");
			String streamDescription = srow.get("description");
			String device = srow.get("device");
			String aggregationInterval = srow.get("aggregationInterval");
			String aggregationType = srow.get("aggregationType");
			String units = srow.get("units");			
			
			Stream s = new Stream();
			s.setStreamId(tableName);
			s.setTableName(tableName);
			s.setVirtual(virtual);
			s.setProcessed(processed);
			s.setStreamDescription(streamDescription);
			s.setDevice(device);
			s.setAggInterval(aggregationInterval);
			s.setAggType(aggregationType);
			s.setUnits(units);			
	...
	public Device addDevice(Device d) {
		return registeredDevices.put(d.getDeviceId(), d);
	}
