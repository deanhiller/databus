...
public class ActionInvoker {
	...
	private static final org.slf4j.Logger log = LoggerFactory.getLogger(ActionInvoker.class);
    ...
    public static void resolve(Http.Request request, Http.Response response) {
        ...
        Params current = Scope.Params.current();
        if(request.resolved && current != null) {
        	...
        	//okay, so on another thread, this broke things so we had to add current != null meaning if Params has already been set AND resolved
        	//so that we ensure execution still.
            return;
        }
        ...
        try {
            ...
            try {
                ...
                if (actionResult == null) {
                    ...
                    try {
                    	//IF there was a chunk failure, skip calling the controller method and throw the chunk failure here
                    	if(request.chunkFailure != null) {
                    		throw new InvocationTargetException(request.chunkFailure);
                    	}
